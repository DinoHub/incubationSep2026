"""Full-stack authentication and authorization check for CountryInfo.
...
ROLE_READ = os.environ.get("ROLE_READ", "countries:read")
ROLE_WRITE = os.environ.get("ROLE_WRITE", "countries:search")
...
# an identity holding the role must get past the role check, one without it must
# be refused with the role named. Every tool here is read-only, so all four rows
# are harmless to run for real.
EXPECTATIONS: list[tuple[str, str, dict]] = [
    ("list_countries", ROLE_READ, {}),
    ("get_country", ROLE_READ, {"name_or_code": "FRA"}),
    ("search_by_currency", ROLE_WRITE, {"currency_code": "EUR"}),
    ("search_by_language", ROLE_WRITE, {"language": "spanish"}),
]


def passed_role_check(message: str, role: str) -> bool:
    """Did this call get past the role boundary, whatever happened afterwards?

    A role denial always names the role it wanted, so its absence means the
    caller was let through. The distinction matters because these tools call an
    external API: with no RESTCOUNTRIES_API_KEY configured, an allowed call
    still fails — on the upstream key, not on authorization. Asserting
    `not is_error` here would report a missing API key as a broken auth
    boundary, which is exactly the confusion this test exists to prevent.
    """
    return role not in message
...
async def main() -> None:
    ...
    async with connect(renewed["access_token"]) as client:
        ...
        r2 = await client.call_tool(EXPECTATIONS[0][0], EXPECTATIONS[0][2]) if EXPECTATIONS else None
        if r2 is not None:
            check("the renewed token still carries the caller's roles",
                  passed_role_check(text_of(r2), EXPECTATIONS[0][1]),
                  text_of(r2)[:160])
    ...
    for who, (client_id, secret, held) in IDENTITIES.items():
        ...
        async with connect(token) as client:
            ...
            for tool, role, args in EXPECTATIONS:
                ...
                msg = text_of(r)
                if role in held:
                    check(f"{who} is allowed past the role check on {tool} "
                          f"(holds {role})",
                          passed_role_check(msg, role), msg[:160])
                    if r.is_error:
                        # Allowed, then failed for another reason — almost always
                        # a missing RESTCOUNTRIES_API_KEY. Not an auth failure,
                        # but worth seeing rather than hiding.
                        print(f"      note: {tool} was allowed but returned an "
                              f"error: {msg[:120]}")
                else:
